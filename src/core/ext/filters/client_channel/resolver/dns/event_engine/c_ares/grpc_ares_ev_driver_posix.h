...
class GrpcPolledFdPosix : public GrpcPolledFd {
 public:
  GrpcPolledFdPosix(ares_socket_t as, EventHandle* poller_handle)
 ...
 private:
  ...
  EventHandle* poller_handle_;
};

class GrpcPolledFdFactoryPosix : public GrpcPolledFdFactory {
 public:
  explicit GrpcPolledFdFactoryPosix(
      absl::AnyInvocable<EventHandle*(ares_socket_t)> on_register)
      : on_register_(std::move(on_register)) {}

  GrpcPolledFd* NewGrpcPolledFdLocked(ares_socket_t as) override {
    return new GrpcPolledFdPosix(as, on_register_(as));
  }

  void ConfigureAresChannelLocked(ares_channel /*channel*/) override {}

 private:
  absl::AnyInvocable<EventHandle*(ares_socket_t)> on_register_;
};
